...
JNIEXPORT jboolean JNICALL Java_kodkod_engine_satlab_MiniSatProver_solveAndAssume
  (JNIEnv * env, jobject, jlong solver, jintArray assumps) {
    jsize length = env->GetArrayLength(assumps);
    jint* buf = env->GetIntArrayElements(assumps, JNI_FALSE);
    vec<Lit> lits;
    bool status;
    for(int i = 0; i < length; ++i) {
        int var = *(buf+i);
    ...
    status = ((Solver*)solver)->solve(lits);
    env->ReleaseIntArrayElements(assumps, buf, 0);
    return status;
  }
