/*-------------------------------------------------------------------------*/
/* Copyright 2013-2014 Armin Biere Johannes Kepler University Linz Austria */
/*-------------------------------------------------------------------------*/
...
struct Yals {
  ...
  STACK(double) scores; STACK(int) cands;
...
static int yals_pick_literal (Yals * yals, int cidx) {
  ...
  for (p = lits; (lit = *p); p++) {
    ...
    LOG ("literal %d weighted break %u score %g", lit, w, s);
    PUSH (yals->scores, s);
    PUSH (yals->cands, lit);
  }

  lit = yals_pick_by_score (yals);
  LOG ("picked literal %d with score %g", lit, s);
...
static void yals_connect (Yals * yals) {
  ...
  for (cidx = 0; cidx < yals->nclauses; cidx++) {
    ...
    while ((lit = *p++)) {
      occsptr = yals_refs (yals, lit), occs;
      occs = *occsptr;
      assert_valid_occs (occs);
...
static void yals_envopt (Yals * yals, const char * name, Opt * opt) {
  int len = strlen (name) + strlen ("YALS") + 1, val;
  char * env = yals_malloc (yals, len), * p, ch;
  const char * str;
...
void yals_stats (Yals * yals) {
  ...
  yals_msg (yals, 0,
    "%lld flips, %.1f thousand flips per second",
