...
public class PreferencesDialog extends JFrame {
    ...
    protected Iterable<SatSolver> testSolvers() {
        ...
        // FIXME z3 / coverage
        // /tmp/AMALGAMALPHA-transclosure/binary/libz3java.so: libz3.so: cannot open shared object file: No such file or directory
        // amalgam knows libz3.so in the folder next to libz3java.so, but libz3java.so does not
        //loadLibrary("z3");
        //loadLibrary("z3java"); 
    ...
    private static boolean _loadLibrary(String library) {
        try { System.loadLibrary(library);      return true; } catch(UnsatisfiedLinkError ex) { System.err.println(ex.getMessage());}
    ...
    public static void logPrefChanged(SwingLogPanel log, Pref<?> pref) {
        ...
        log.flush();
        // MODIFY log option/preference changes
        Logger.log(Logger.Event.OPTS, "prf="+pref.title, "val="+pref.get());
    }
